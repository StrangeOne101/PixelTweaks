    ...
    public boolean parse(Consumer<PokemonSpecification> consumer, String s) {
        try {
            ...
            if (s.charAt(0) == '*') { //Override the parsing of the spec
                if (consumer != null) consumer.accept(PokemonSpecificationProxy.create(s.substring(1)).get());
                return true;
            }

            PokemonSpecification pokemonSpecification = PokemonSpecificationProxy.create(s).get();
