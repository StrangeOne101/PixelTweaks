...
public class EventRegistry implements ISelectiveResourceReloadListener {
    ...
    protected static void registerEvent(Event event) {
        EVENTS.computeIfAbsent(event.getClass(), k -> Sets.newHashSet()).add(event);
    }
    ...
    public EventRegistry() {
        ...
        if (Minecraft.getInstance().getResourceManager() instanceof IReloadableResourceManager) {
            ...
            PixelTweaks.LOGGER.info("Failed to add music loader");
        }
    ...
    public void onResourceManagerReload(IResourceManager resourceManager) {
        String directoryPath = "pokemusic";

        PixelTweaks.LOGGER.info("Loading music from " + directoryPath);
        ...
        for (ResourceLocation location : fileLocations) {
            PixelTweaks.LOGGER.info(location.toString());
            try {
        ...
        PixelTweaks.LOGGER.info("Loaded " + fileLocations.size() + " music files");
