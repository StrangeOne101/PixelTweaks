...
# 1.10.0 (PokeChat + Updates)
- Updated PixelTweaks for Pixelmon 9.1.8/9.1.9
- Added new Submit Pokemon task in FTB Quests
  - Consumes a pokemon when submitted to the quest
  - Has the option for the pokemon to match the OT of the user
  - Can select where it removes the pokemon from (party, box, or both)
- Added new Release pokemon task in FTB Quests
  - For when a player releases a pokemon
  - Also has the option for the pokemon OT to match
- Added PokeChat - Display your party pokemon in chat!
  - `[pokemon]` displays your selected pokemon
  - `[pokemon<1-6>]` displays the specific slot of pokemon. E.g. `[pokemon3]`
  - Will display an icon of the pokemon when you hover over it, along with the stats
  - Language friendly! Works in all languages
- Updated TravelersBackpack Integration
  - Requires TravelersBackpack 5.4.47 and above
- Backported FTB Quest 1.20 feature where larger GUI scales divide the quest menu into multiple columns
- Fixed some pokemon names showing an incorrect lang node for the default title of some pokemon tasks with certain pokemon (gastrodon, ho-oh, etc)
- Fixed random pokemon specs giving the same pokemon each time for FTB quest pokemon rewards
- Improved autogenerated titles for quests when using `shiny:false` and eggs
